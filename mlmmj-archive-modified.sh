...
# Just few tweak been done by <error@oddprotocol.org>
...
_conffile="/etc/mlmmj-archivist/mlmmj-archivist.conf"
...
if [ ${_language} != 'en_US' ]; then
	...
	test -d "/usr/local/share/mlmmj-archivist/templates/${_template}.${_html_lang}" &&
		_template="${_template}.${_html_lang}" \

	# check if translated strings are available and source it
	test -s "/usr/local/share/mlmmj-archivist/templates/${_template}/trans_str.txt" &&
		. "/usr/local/share/mlmmj-archivist/templates/${_template}/trans_str.txt"
fi

# set the template
_mhontemplate="/usr/local/share/mlmmj-archivist/templates/${_template}/mhonarc/template.mrc"
[ -s "${_mhontemplate}" ] \
...
do
	...
	while [ ${_msg} -le ${_curindex} ]; do
		...
		mhonarc -rcfile /etc/mlmmj-archivist/mhonarc.mrc ${_mhonarc_args}  \
	...
	rsync -aq --delete --delete-after                   \
		/usr/local/share/mlmmj-archivist/templates/${_template}/assets/ \
		${_public_html}/assets/
	...
	sed     -e "s@__HTMLLANG__@${_html_lang}@g"               \
		...
		/usr/local/share/mlmmj-archivist/templates/${_template}/listinfo.tmpl \
		> ${_temp_listinfo}
	...
	do
		...
		_content="${_content}\</tbodys\>\</table\>"
		_content="${_content}\</diva\><!-- .month-list -->"
	done
	...
	sed     -e "s@__HTMLLANG__@${_html_lang}@g"               \
		...
		-e "s@__PUBURL__@${_public_url}@g"                \
		-e "s@__CONTENT__@${_content}@g"                  \
		/usr/local/share/mlmmj-archivist/templates/${_template}/listpage.tmpl \
		> ${_temp_mainindex}
...
then
	...
	if ${_metadata} && [ ${_template} = 'ellak' ]; then
		...
		_content="${_content}\<p class=\"listmeta\"\><small>${_str_meta_sub:-subscribers}: <span>${_wikielsub}</span> | ${_str_meta_msg:-messages}: 
<span>${_wikielmsg}</span></small></p>"
	fi
	...
	for _mlist in $(echo ${_mlists}); do
		...
		if ${_metadata}; then
			...
			_content="${_content}\<p class=\"listmeta\"\><small>${_str_meta_sub:-subscribers}: <span>${_meta_subs}</span> | 
${_str_meta_msg:-messages}: <span>$(cat ${_listdir}/index)</span></small></p>"
		fi
	done
	...
	sed     -e "s@__HTMLLANG__@${_html_lang}@g"               \
		-e "s@__PUBURL__@${_public_url}@g"                \
		-e "s@__CONTENT__@${_content}@g"                  \
		/usr/local/share/mlmmj-archivist/templates/${_template}/homepage.tmpl \
		> ${_temp_homeindex}
